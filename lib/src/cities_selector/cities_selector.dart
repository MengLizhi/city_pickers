...
class CitiesSelector extends StatefulWidget {
  ...
  /// 定义右侧bar的激活与普通状态的颜色
  Color? tagBarBgColor;
  Color? tagBarActiveColor;

  /// 定义右侧bar的字体的激活与普通状态的颜色
  final Color tagBarFontColor;
  Color? tagBarFontActiveColor;
...
//  final double cityItemHeight;
  ...
  Color? topIndexBgColor;

  Color? itemSelectFontColor;
...
//      FocusScope.of(context).requestFocus(FocusNode());
      _dynamicChangeTopStagePosition(( _scrollController?.offset.toDouble() ?? 0 ));
    });
    super.initState();
  ...
  List<CityOffsetRange> _initOffsetRangList() {
    if (_offsetTagRangeList.isEmpty) {
      double itemContainerHeight =
          ( _key0.currentContext?.findRenderObject() as RenderBox ).paintBounds.size.height;
              ...
              Container(
                ...
                child: Center(
                  child: ListTileTheme(
                    ...
                    selectedColor:
                        widget.itemSelectFontColor,
                    textColor: widget.itemFontColor,
                    child: ListTile(
                      selected: selected,
